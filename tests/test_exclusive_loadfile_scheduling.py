...
from xdist_scheduling_exclusive.exclusive_loadfile_scheduling import (
    ExclusiveLoadFileScheduling,
    EXCLUSIVE_TEST_SCOPE_PREFIX,
)
...
def exclusive_tests_mock():
    with patch(
        "xdist_scheduling_exclusive.exclusive_loadfile_scheduling.load_exclusive_tests"
    ) as mock:
...
def test_exclusive_loadfile_split_scope_regular(exclusive_tests_mock, config_mock, log_mock):
    ...
    with patch(
        "xdist_scheduling_exclusive.exclusive_loadfile_scheduling.LoadFileScheduling._split_scope"
    ) as super_split_scope_mock:
